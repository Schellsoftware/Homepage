...
public class Schweinewachstum extends Applet implements Runnable{
	...
	String str;
	Image schwein,aschwein,schnitzel;
	int zeit=0;
	int px[] = new int[3];
	int py[] = new int[3];
	public void init()
	{
		...
		g=null;
		resize(400, 200);
		schnitzel = getImage(getCodeBase(),"schnitzel.jpg");
		schwein = getImage(getCodeBase(),"schwein.jpg");
		MediaTracker mt = new MediaTracker(this);
		mt.addImage(schwein,0);
		mt.addImage(schnitzel,0);
		try {
			mt.waitForAll();
	...
	public void rechneZyklus()
	{
		...
		zeit+=5;
		if(zeit>250) zeit=0;
		aschwein=schwein.getScaledInstance(50+zeit*91/400,50+zeit*91/400,Image.SCALE_FAST);
	...
	public void zeichneZyklus()
	{
		...
		g.setColor(Color.black);
		g.drawString("Harald Schellinger",20,180);
	}
