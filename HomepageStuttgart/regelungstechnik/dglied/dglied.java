 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class dglied extends Applet implements Runnable{
	...
	public void rechneZyklus()
	{
		...
		if (zeit >=1)
		{
			y[zeit]=(int)(3*((double)x[zeit]-(double)x[zeit-1]));
		}
		for(int i=149;i>0;i--)
		{
		//	x[i]=x[i-1];
		}
		yk=ykm1+(double)x[0]/100;
		ykm1 = yk;
		//y[0]=(int)(100*(x[1]-x[0]));
		for(int i=149;i>0;i--)
		{
	...
	public void zeichneZyklus()
	{
		...
		g.drawLine(20+zeit,150-x[zeit]+y[zeit]*10,40+zeit,150-x[zeit]-y[zeit]*10);
		...
		g.drawString("Periodendauer",165,35);
